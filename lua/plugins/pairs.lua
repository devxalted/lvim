return {
	{
		...
		config = function(_, opts)
			-- Snacks toggle is optional — don't let it block pairs from loading
			pcall(function()
				Snacks.toggle({
					name = "Mini Pairs",
			...
			pairs.open = function(pair, neigh_pattern)
				...
				-- Always pair { after $ for template literal interpolation ${...}
				if o == "{" and before:sub(-1) == "$" then return open(pair, neigh_pattern) end
